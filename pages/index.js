...
function Home() {
  ...
  // check for connection authentication before allowing user to login to system
  const checkAuth = ({ link }) => {
    setModal()
    if (isAuthenticated) {
      setModal()
  ...
  return (
    <>
      <div className="relative">
        <video autoPlay loop muted className="object-cover h-screen w-screen">
          <source src="/home.mp4" type="video/mp4" />
        </video>

        <div className="absolute top-0 right-0 w-screen h-screen bg-gradient-to-tr from-slate-900/80 via-indigo-900/80 to-slate-900/60 flex flex-col gap-6 justify-center items-center px-2">
          <span className="flex flex-col justify-center items-center">
            <div className="w-14 lg:w-20 top-2 lg:top-8 mb-3">
              <Image
                src={"/glogo.png"}
                layout="responsive"
                width={512}
          ...
          {modal && (
            <div
              className="p-2 mb-4 text-sm text-yellow-700 bg-yellow-100 rounded-lg dark:bg-yellow-200 dark:text-yellow-800 flex justify-center items-center gap-2"
              role="alert"
            >
              <span className="font-medium">😤</span> {modal}{" "}
              <XIcon onClick={() => setModal(!modal)} className="w-6" />
            </div>
          )}
          <div className="relative mb-3">
            ...
            >
              <Image
                ...
                className="object-cover rounded-full"
              />
              <small className="bg-slate-900 absolute bottom-0 left-6 px-2 rounded-full text-white">
                Help
              </small>
            </span>
            <span
            ...
            >
              <Image
                ...
                className="object-cover rounded-full"
              />
              <small className="bg-slate-900 absolute bottom-0 left-4 px-2 rounded-full text-white">
                Admin
              </small>
            </span>
          ...
          <span className="flex items-center flex-col">
            {!toggleLoading && !user && (
              <button
                type="button"
                className="text-gray-300 group bg-gradient-to-r from-purple-500 via-purple-600 to-purple-700 hover:bg-gradient-to-br border border-purple-400 focus:ring-4 focus:outline-none focus:ring-purple-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 mr-2 mb-2"
                onClick={() => connect()}
              >
                <svg
                  className="w-6 h-5 mr-2 -ml-1 cursor-pointer group-hover:scale-105 group-hover:-rotate-6 duration-150"
